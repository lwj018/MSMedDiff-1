...
class SCMA(nn.Module):
    def __init__(self, in_channels, rate=4):
        ...
        self.channel_attention = nn.Sequential(
            #KANLinear(in_channels, int(in_channels / rate)),
            nn.Linear(in_channels, int(in_channels / rate)),  # 线性层，将通道数缩减到1/rate
            nn.ReLU(inplace=True),  # ReLU激活函数
            #KANLinear(int(in_channels / rate), in_channels),
    ...
    def forward(self, x):
        ...
        x_att_permute = self.channel_attention(x_permute).reshape(b, h, w, c)  # 应用通道注意力
...
class KSCMA(nn.Module):
    def __init__(self, in_channels, rate=4):
        super(KSCMA, self).__init__()
